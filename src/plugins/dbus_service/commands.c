...
static gboolean __lambda3_ (UsageShutdown* self);
...
static gboolean __lambda4_ (UsageReboot* self);
...
static gboolean usage_shutdown_run_co (UsageShutdownRunData* _data_) {
	...
	g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, ___lambda3__gsource_func, usage_system_command_ref (_data_->self), usage_system_command_unref);
	if (_data_->_state_ == 0) {
		g_simple_async_result_complete_in_idle (_data_->_async_result);
	} else {
		g_simple_async_result_complete (_data_->_async_result);
...
static gboolean usage_reboot_run_co (UsageRebootRunData* _data_) {
	...
	_state_2:
	usage_controller_disableAllResources_finish (_data_->_tmp1_, _data_->_res_);
	g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, ___lambda4__gsource_func, usage_system_command_ref (_data_->self), usage_system_command_unref);
	if (_data_->_state_ == 0) {
		g_simple_async_result_complete_in_idle (_data_->_async_result);
	} else {
		g_simple_async_result_complete (_data_->_async_result);
